---
name: fastify
description: "Fastify backend and HTTP API patterns for GridBoard. Use when building or changing Fastify routes, plugins, Zod/JSON Schema validation, error handling, auth hooks, asset endpoints, health checks, WebSocket handoff, PostgreSQL/Drizzle access, Redis/BullMQ job APIs, or production server configuration. Trigger terms: Fastify, server.ts, API route, route handler, plugin, request lifecycle, reply, validation, CORS, health check."
metadata:
