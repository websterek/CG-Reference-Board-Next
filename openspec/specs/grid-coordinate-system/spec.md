...
The grid is the coordinate system for all structured items on a board. Item positions and sizes are stored as integer multiples of the configured cell size in board coordinates. The grid is drawn in world (board) coordinates so the parent zoom transform handles scaling; grid lines remain visible at every zoom level. Snap and overlap checks operate on board coordinates so zoom level never affects accuracy.

---

<!-- Promoted from openspec/changes/grid-native-items-and-typed-layers/specs/grid-coordinate-system/spec.md -->
