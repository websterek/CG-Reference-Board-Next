...
Two interaction modes (Grid Mode and Annotation Mode) with different tool sets and snap behavior. Mode is UI state in Zustand `uiStore`; switching mode swaps which tools are available without mutating any item. Annotation Mode disables snap so free-draw strokes store raw board coordinates.
